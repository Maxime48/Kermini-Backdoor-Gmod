...
class Server
{
	...
	// Ajoute un serveur
	public function AddServer($name, $ip, $users, $server_back_door)
	{
		$GLOBALS['DB']->Insert("servers", ["HostName" => $name, "IPAddress" => $ip, "Players" => $users, "LastUpdate" => time(), "Backdoor_Type" => $server_back_door]);
	}

	// Mets à jour un serveur
	public function UpdateServer($server_id, $name, $ip, $users, $server_back_door)
	{
		$GLOBALS['DB']->Update("servers", ["id" => $server_id], ["HostName" => $name, "Players" => $users, "LastUpdate" => time(), "Backdoor_Type" => $server_back_door]);
	}
